...
include_once(__DIR__ . '/init_code.php');

session_start();

define('SIZE_LIMIT', 40000); // Tamaño límite de la imagen // 40Mb (por poner un valor)

define('SUPPORTED_MIMETYPES', [
    'image/jpeg',
    'image/gif',
    'image/png',
    'image/bmp',
...
//TODO Sacarlo al archivo functions.php
// OJO No se si es necesario si ya se hace la comprobación en el cliente
...
{
    return strlen($password) >= 8 && preg_match('/[A-Za-z].*[0-9]|[0-9].*[A-Za-z]/', $password);
}
    ...
    if ($account) {
        ...
        if ($_POST['pwd'] == $_POST['pwd_chck']) {
            ...
            if (isset($_FILES['profile_img']) && is_uploaded_file($_FILES['profile_img']['tmp_name'])) {
                ...
                $is_correct_format = in_array($_FILES['profile_img']['type'], SUPPORTED_MIMETYPES);
                ...
                if ($is_correct_format && $_FILES['profile_img']['size'] < SIZE_LIMIT) {
                    // TODO Se puede obviar el formato de la imagen haciendo un preprocesamiento en la parte cliente con JS y una normalización en la parte PHP
                    ...
                    // OJO Esto se puede mejorar mucho con un sistema de conversión de imagenes (imagewebp) - https://www.php.net/manual/en/function.imagewebp.php
            ...
            $hash = hash('sha1', $random_confirm_code);
