    ...
    cart: function () {
        ...
        listWrapper.addEventListener("click", (e) => {
            const targetEl = e.target,
                prod_id = targetEl.closest(".list_product").dataset.id;

            if (targetEl.id == "") {
                if (targetEl.tagName == "IMG") {
                    window.location = `/DAW_proyecto_final/templates/product_detail.php?id=${prod_id}`;
                } else if (targetEl.tagName == "BUTTON") {
                    // TODO Mover a la cesta: eliminar de la lista y añadir al carrito
                } else if (
                    targetEl.tagName == "A" &&
        ...
        items_container.addEventListener("click", (e) => {
            ...
            if (targetEl.classList.length > 0) {
                switch (targetEl.classList.value) {
                    ...
                    case "cart_card_cta_btn":
                        if (targetEl.dataset.action == "delete") {
                            ...
                            if (!prodAlreadySaved) {
                                ...
                                // TODO Agregar elemento a la lista de para comprar más tarde
                            }
                        }
                    ...
                    case "prod_qty_wrapper":
                        prod_qty_wrapper_clicked = targetEl;

                        actualQty = parseInt(
                            targetEl.getElementsByClassName(
                                "prod_qty_number"
                            )[0].innerHTML
                        );
                        showQtyDropdown(targetEl, actualQty);
                        break;
                    ...
                    default:
                        // Si es .prod_qty_number
                        if (
                            targetEl.nodeName == "SPAN" &&
                            targetEl.parentElement.classList.value ==
                                "prod_qty_wrapper"
                        ) {
                            actualQty = parseInt(targetEl.innerHTML);
                            prodSize =
                                targetEl.closest(".cart_item").dataset.prodSize;
                ...
                // Si es un span que no es .prod_qty_number o img
                prod_qty_wrapper_clicked = targetEl.parentElement;
                if (
                    prod_qty_wrapper_clicked.classList.value ==
                    "prod_qty_wrapper"
                ) {
                    if (targetEl.nextElementSibling != null) {
                        actualQty = parseInt(
...
function updateTotals({ nextStatus = null, newQty, prod_id, prodSize }) {
    ...
    if (nextStatus != null) {
        ...
        console.log(totalPrice, totalProds);
    } else {
